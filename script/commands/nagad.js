...
module.exports.config = {
  ...
  premium: false,
  category: "Fun",
  usages: "nagad number-txid-amount-charge",
  cooldowns: 5
};
...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (!args[0]) {
    ...
    return api.sendMessage(
      "❌ Usage: example number-txid-amount-charge\nExample: example 01912345678-ABC123XYZ-5000-10",
      threadID,
      messageID
    );
  }
  ...
  const url = `https://masterapi.site/api/nagadf.php?number=${encodeURIComponent(number)}&transaction=${encodeURIComponent(transaction)}&amount=${amount}&charge=${charge}&total=${total}`;
